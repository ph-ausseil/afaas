...
from autogpt.core.memory.table.base import BaseTable, AgentsTable, MessagesTable, UsersTable
...
def test_messages_table_list():
    ...
    assert result == data_list[::-1]  # Assuming data_list is sorted by 'timestamp' in ascending order
...
def test_users_table_list():
    ...
    assert result == data_list  # Assuming data_list is sorted by 'username' in ascending order
