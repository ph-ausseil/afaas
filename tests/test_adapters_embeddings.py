...
@pytest.mark.asyncio
async def test_get_related_documents_test_query_no_result(vector_store_wrapper_with_documents : VectorStoreWrapper):
...
@pytest.mark.asyncio
async def test_get_related_documents_no_matching_results(vector_store_wrapper_with_documents : VectorStoreWrapper):
    search_filter = SearchFilter(filters={"agent_id": Filter(filter_type=FilterType.EQUAL, value="nonexistent")})
    results = await vector_store_wrapper_with_documents.get_related_documents(
        query= "test query",
        nb_results=5,
        search_filters=search_filter,
...
def test_get_related_documents_invalid_filter():
    with pytest.raises(ValidationError) as excinfo:  
        SearchFilter(filters={"invalid_filter": Filter(filter_type="invalid_type", value="123")})


@pytest.mark.asyncio
async def test_get_related_documents_cluster_search(vector_store_wrapper_with_documents : VectorStoreWrapper):
    search_filter = SearchFilter(filters={"agent_id": Filter(filter_type=FilterType.EQUAL, value="123")})
    results = await vector_store_wrapper_with_documents.get_related_documents(
        query="test query",
        nb_results=5,
