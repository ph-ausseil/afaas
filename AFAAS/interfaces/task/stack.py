...
class TaskStack(AFAASModel):
    ...
    def validate(cls, v: Any) -> "TaskStack":
        ...
        if isinstance(v, cls):
            ...
            return cls(
                **v
            )  # Adjust this line as needed based on how TaskStack is initialized
        else:
    ...
    def add(self, task: AbstractTask):
        ...
        """
        LOG.debug(
            f"Adding task {LOG.italic(task.debug_formated_str())})in a stack of task {LOG.italic(self.parent_task.debug_formated_str())}"
        )
    ...
    async def get_all_tasks_from_stack(self) -> list[AbstractTask]:
        ...
        """
        return [
            await self.parent_task.agent.plan.get_task(task_id) for task_id in self._task_ids
        ]
    ...
    async def get_ready_tasks_from_stack(self) -> list[AbstractTask]:
        ...
        return [
            await  self.parent_task.agent.plan.get_task(task_id) for task_id in common_task_ids
        ]
